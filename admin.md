...
**Overview of administrative tasks for Health Record Ingestion:** 
...
[Onboarding Data Integrators](#onboarding-new-data-integrators)
      ...
      - [Using the management API stream endpoints](#using-the-management-api-stream-endpoints)
...
Before proceeding, make sure that your IBM&reg; Cloud [Elasticsearch (Document Store)](glossary.md#elasticsearch) account includes either:
      * An existing Service Credential you can access 
  ...
  **Note:** This requires the use of the [command-line cURL](https://github.com/curl/curl) tool.
...
2. In Elasticsearch, in the left menu, click **Service Credentials**.

**Figure 1: Elasticsearch menu**
...
7. For Curl to authenticate with the IBM Cloud Elasticsearch instance, download the certificate and export it. To do this, navigate to the Management screen for your Elasticsearch instance.
   ...
   **Figure 3: Example of the Management screen**
...
You can now interact with the Elasticsearch REST API using Curl. 

**Example** 

Here's how you can get the status of the cluster, perform a `GET` on the `_cluster/health?pretty` endpoint:
...
**To load an index template into Elasticsearch:**
...
There are four Management API endpoints that support Tenant Management in Elasticsearch for Health Record Ingestion: Create, Get (all tenants), Get (specific tenant), and Delete. 

**Note:** Each of these endpoints requires an API key and Identity and Access Management (IAM) Authentication. In requests, you must pass an IAM Bearer token in the Authorization header.
...
Health Record Ingestion has been designed with Health Insurance Portability and Accountability Act (HIPAA) compliance in mind. To satisfy HIPAA data isolation requirements, there must be two event streams (Kafka) topics for every unique combination of tenant and [Data Integrator](glossary.md#data-integrator). For more information, see [Multi-tenancy](multitenancy.md). 

These topics can be added automatically through the Management API. For more information, see [Using the Management API Stream Endpoints](#using-the-management-api-stream-endpoints)).

### About the naming conventions for topics

Health Record Ingestion uses these naming conventions for the two new topics you create:
  `ingest.<tenantId>.<dataIntegratorId>[.optionalMetadataTag].in`
  `ingest.<tenantId>.<dataIntegratorId>[.optionalMetadataTag].notification`

where:

`optionalMetadataTag` is defined by the Data Integrator. Since it is optional, this can be blank. 

**Example** 

With the tenant ID `24` and Data Integrator ID `producer`, the resulting two topics are `ingest.24.producer.in` and `ingest.24.producer.notification`. In addition, the tenant ID can include a `tenant` prefix, for example, `ingest.tenant24.producer.in` and `ingest.tenant24.producer.notification`. However, it must be consistent with the Elasticsearch [index tenant id](#create-a-new-tenant-in-elasticsearch). 

**Note:** Only use lowercase alphanumeric characters, hyphen (-), and underscore (\_\).

### Using the Management API stream endpoints

Setting up Health Record Ingestion for a new Data Integrator also requires creating a [stream](glossary.md#stream). The Management API contains three `Stream` endpoints to help you manage the streams: `Create`, `Get`, and `Delete`. 
...
For the tenant and Data Integrator pairing, you can use the Management API Create Stream endpoint to create the `in` and `notification` topics.
...
For the `tenantId` **tenant24**, Data Integrator ID **data-int-1**, and optional qualifier **qualifier1**, you could use this curl command:
...
**Note:** The `numPartitions` and `retentionMs` topic configurations are required. You can pass other optional configurations. For information about these optional fields, see the Health Record Ingestion [API specification](apispec.md).
...
**Important:** Topic naming conventions for Health Record Ingestion require topics to start with the prefix **ingest** and end with either the suffix **in** or **notification**. **Both the Get and Delete endpoints ignore any topics that do not follow this convention**.
...
You could use `ingest.24.integrator1.*` to allow access to both the `ingest.24.integrator1.in` topic and the `ingest.24.integrator1.notification` topic.

The Data Integrator needs read and write access to the input topic. However, the Data Integrator only needs read access to the notification topic. This requires a total of four IAM policies. 

The downstream Consumer only needs read access to the input and notification topics. This requires a total of three IAM policies. 
...
Any user of the Management API, for example, Data Integrators or Consumers, needs an API key. The Management API in Health Record Ingestion uses [IBM Cloud Functions'](glossary.md#ibm-cloud-functions) built in API key management. 
...
2. Click the `hri` API. This opens a window with details about the API. 
