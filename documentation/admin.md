...
The tenant ID must be consistent with the Elasticsearch [index tenant ID](#managing-tenants). Only use lowercase alphanumeric characters, `-`, and `_`.
...
- The `numPartitions` parameter is applied to the `*.in` topic, but the `*.notification` topic is set to 1.
...
**Note**: policies support wildcards at the beginning and/or end of the 'Resource ID' field when using the 'string matches' qualifier. This enables _a single policy to allow access to multiple topics_ when they share a common substring. For example, `ingest.24.data-int-1.*` could be used to allow access to the `ingest.24.data-int-1.in` and `ingest.24.data-int-1.notification` topics.
